...
test('lists with index suffix', assert => {
  ...
  andThen(() => {
    assert.equal(findWithData('panel').size(), 2, 'All panel elements are found');

    let $container = findWithData('container');
    assert.equal($container.size(), 1, 'container element is found');

    let $nestedPanel = findWithData('panel', $container);
    assert.equal($nestedPanel.size(), 1, 'only 1 nested panel is found');
